...
class Refundreturn extends BaseModel {
    ...
    public function refundAmount($order, $refund_amount) {
        ...
        // print_r($order);die();
        $pt_amount = $order['pt_amount']; //支付的犀豆（积分）金额 漫步2023.6.24
        
        // echo 999;die();

        $predeposit_amount = $order_amount - $order['refund_amount'] - $rcb_amount - $pt_amount; //可退预存款金额（预存款+在线支付金额） 在线支付可能原路返还
        ...
        if($pt_amount>0){
            ...
            $log_array['amount'] = $refund_amount;
            if ($pt_amount > 0) {
                $log_array['amount'] = $pt_amount;
            }
            
            // echo $refund_amount;
            // echo $pt_amount;die();
            // print_r($log_array);die();
            
            $state = $predeposit_model->changePt('refund', $log_array); //增加买家可用犀豆（积分）
            if (!$state) {
                throw new \think\Exception('充值卡退回失败', 10006);
            }
        }
    ...
    public function editOrderRefund($refund) {
        ...
        if ($refund_id > 0) {
            ...
            $field = 'order_id,buyer_id,buyer_name,store_id,order_sn,order_amount,payment_code,order_state,refund_amount,rcb_amount,pd_amount,pt_amount,trade_no';
            $order_model = model('order');
