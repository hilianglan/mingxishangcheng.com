...
class Points extends BaseModel {
     ...
     * @access public
     * @author csdeshang
     * @param  string $stage 操作阶段 regist(注册),login(登录),comments(评论),order(下单),system(系统),other(其他),pointorder(积分礼品兑换),app(同步积分兑换),pointtofriend(积分转给好友),pointfromfriend(积分来自好友)
    ...
    function savePointslog($stage, $insertarr, $if_repeat = true) {
        ...
        switch ($stage) {
            ...
            case 'pointtofriend':
                if (!isset($insertarr['pl_desc'])) {
                    $insertarr['pl_desc'] = '积分转给好友';
                }
                break;
            case 'pointfromfriend':
                if (!isset($insertarr['pl_desc'])) {
                    $insertarr['pl_desc'] = '积分来自好友';
                }
                break;
            case 'marketmanage':
                break;    
        ...
        if (isset($insertarr['pl_memberfriendid'])) {
            $value_array['pl_memberfriendid'] = $insertarr['pl_memberfriendid'];
        }
        if (isset($insertarr['pl_memberfriendname'])) {
            $value_array['pl_memberfriendname'] = $insertarr['pl_memberfriendname'];
        }
