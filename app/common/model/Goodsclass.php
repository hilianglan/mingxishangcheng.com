...
class Goodsclass extends BaseModel
{
    ...
    {
        // $result = Db::name('goodsclass')->field($field)->where($condition)->order('gc_parent_id asc,gc_sort asc,gc_id asc')->select()->toArray();//hehao注释
        $result = Db::name('goodsclass')->field($field)->where($condition)->order('gc_parent_id asc,gc_sort asc,gc_id asc')->select()->toArray();//hehao修改
        return $result;
    }
