...
class SellerMiniproLive extends MobileSeller {
    ...
    public function get_room_list(){
        ...
        foreach($room_list as $key => $val){
                $room_list[$key]['goods_list']=$minipro_live_room_goods_model->getMiniproLiveRoomGoodsList(array(array('minipro_live_id','=',$val['minipro_live_id'])));
                foreach($room_list[$key]['goods_list'] as $k => $v){
                    $room_list[$key]['goods_list'][$k]['goods_image_url']=goods_cthumb($v['goods_image'], 480, $v['store_id']);
                }
        }
    ...
    public function upload_media() {
        ...
        $file_name='';
        if(input('param.save')){
            $res = ds_upload_pic(ATTACH_MINIPRO_LIVE, 'file');
            if (!$res['code']) {
                ds_json_encode(10001, $res['msg']);
            }
