...
class Member extends MobileMember {
    ...
    public function index() {
        ...
        $member_info['order_nopay_count'] = intval($order_model->getOrderCountByID('buyer', $this->member_info['member_id'], 'NewCount'));
        $member_info['order_noreceipt_count'] = intval($order_model->getOrderCountByID('buyer', $this->member_info['member_id'], 'SendCount'));
        $member_info['order_noeval_count'] = intval($order_model->getOrderCountByID('buyer', $this->member_info['member_id'], 'EvalCount'));
        $member_info['order_noship_count'] = intval($order_model->getOrderCountByID('buyer', $this->member_info['member_id'], 'PayCount'));
        $member_info['order_refund_count'] = intval($refundreturn_model->getRefundreturnCount(array(array('buyer_id', '=', $this->member_info['member_id']), array('refund_state', 'in', [1, 2]))));
    ...
    public function goods_poster() {
        ...
        if (!file_exists($refer_qrcode_logo) || 1) {
            ...
            $logo = imagecreatefromstring(file_get_contents($store_info['store_avatar']));
            $source_info = getimagesize($store_info['store_avatar']);
            imagecopyresampled($t_logo2, $logo, $left_temp, $y, 0, 0, $logo_size, $logo_size, $source_info[0], $source_info[1]);


            //将店铺名称放到白板上
            $text = $store_info['store_name'];
