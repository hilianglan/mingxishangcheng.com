...
//        }
        ...
        if (is_array($member_info) && !empty($member_info)) {
            ...
            if ($member_info['is_del'] == '1') {
                ds_json_encode(10001, '账号已注销',array('msg' => '账号已注销'));
   ...
   public function ddel(){
        // ds_json_encode(10001, '注销失败/fail',array('msg' => '注销失败'));
        $un=input('username');
        $member=Db::name('member')->where('member_name',$un)->update(['is_del'=>'1']);
        $res=Db::name('member')->where('member_name',$un)->value('is_del');
        if($res == '1'){
            ds_json_encode(200, '注销成功/success',array('msg' => '注销成功'));
        }else{
            ds_json_encode(10001, '注销失败/fail',array('msg' => '注销失败'));
        }
...
//                    'headimgurl' => $avatar,#提高体验暂时不对图片进行处理
    ...
    public function third_login(){
        ...
        if($id){
            ...
            $result=array('key'=>$key,'user_info'=>$user_info,'wxinfo'=>$wxinfo);
            if($user_info){
                //是否有卖家账户
                $seller_model = model('seller');
                $seller_info = $seller_model->getSellerInfo(array('member_id' => $user_info['member_id']));
                if($seller_info){
                    $token = Sellerlogin::_get_seller_token($seller_info['seller_id'], $seller_info['seller_name'], 'wap');
                    if(!$token){
                        ds_json_encode(10001,lang('login_fail'));
                    }
                    //读取店铺信息
                    ...
                    $result['seller_token'] = $token;
                    $result['seller_info'] = $this->getSellerUser($seller_info,$store_info);
                    $result['user_info']['store_id']=$store_info['store_id'];
                }
            }
            ds_json_encode(10000, '',$result,'',false);
        }else{
