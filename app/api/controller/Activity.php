...
class Activity extends MobileMall {
    ...
    function index()
    {
        ...
        foreach($activity_list as $key => $val){
            ...
            $condition[]=array('activitydetail_state','=',1);
            $goods_list=$activitydetail_model->getGoodsJoinList($condition,3,'activitydetail_sort asc');
            foreach($goods_list as $k => $v){
                $goods_list[$k]['goods_image_url'] = goods_cthumb($v['goods_image'], 480, $v['store_id']);
            }
    ...
    public function detail() {
        ...
        $activitydetail_list = $activitydetail_model->getGoodsJoinList($condition,$this->pagesize,'activitydetail_sort asc','goods.goods_commonid');
        foreach($activitydetail_list as $key => $val){
            $activitydetail_list[$key]['goods_image_url'] = goods_cthumb($val['goods_image'], 480, $val['store_id']);
        }
