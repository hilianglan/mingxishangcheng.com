...
class Shopnearby extends MobileMall {
    ...
    private function _get_Own_Store_List() {
        ...
        if (!empty(input('post.keyword'))) {
            $condition[] = array('store_name', 'like', '%' . input('post.keyword') . '%');
        }
        ...
        $store_list_object = Db::name('store')->where($condition)
        ...
        foreach ($store_list as $key => $value) {
            ...
            $store_list[$key]['goods_list'] = model('goods')->getGoodsListByColorDistinct(array(array('store_id' ,'=', $value['store_id']), array('goods_commend' ,'=', 1)), 'goods_image,goods_id,goods_price', 'goods_id desc', 0, 4);
