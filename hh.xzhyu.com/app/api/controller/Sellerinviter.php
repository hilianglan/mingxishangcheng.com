...
class Sellerinviter extends MobileSeller {
    ...
    public function goods_list() {
        ...
        $goods_list = $goods_model->getGoodsCommonList($condition, '*', $this->pagesize);
    ...
    public function goods_add() {
        ...
        $goods_info = $goods_model->getGoodsCommonInfo('goods_commonid=' . $inviter_goods_commonid . ' AND store_id=' . $this->store_info['store_id']);
        if (!$goods_info) {
    ...
    public function goods_info() {
        ...
        $goods_info = $goods_model->getGoodsCommonInfo('goods_commonid=' . $goods_commonid . ' AND inviter_open=1 AND store_id=' . $this->store_info['store_id']);
        if (!$goods_info) {
            ds_json_encode(10001, lang('sellerinviter_goods_empty'));
        }
    ...
    public function goods_edit() {
        ...
        $goods_info = $goods_model->getGoodsCommonInfo('goods_commonid=' . $inviter_goods_commonid . ' AND inviter_open=1 AND store_id=' . $this->store_info['store_id']);
        if (!$goods_info) {
            ds_json_encode(10001, lang('sellerinviter_goods_empty'));
        }
            if ($inviter_ratio <= 0) {
    ...
    public function search_goods() {
        ...
        $condition = array();
        $condition[] = array('store_id', '=', $this->store_info['store_id']);
        $condition[] = array('inviter_open', '=', 0); //未选择为分销的商品
        $condition[] = array('goods_name', 'like', '%' . input('param.goods_name') . '%');
        $goods_list = $goods_model->getGoodsCommonList($condition, 'goods_commonid,goods_name,goods_price,goods_image,inviter_open', $this->pagesize);
