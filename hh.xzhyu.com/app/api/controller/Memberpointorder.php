...
class Memberpointorder extends MobileMember {
    ...
    public function order_info() {
        ...
        $condition[] = array('point_buyerid','=',$this->member_info['member_id']);
        $order_info = $pointorder_model->getPointorderInfo($condition);
        if (!$order_info) {
    ...
    public function search_deliver() {
        ...
        $express_code = $order_info['point_shipping_ecode'];
        $expresslist = array_filter($express, function($t) use ($express_code) { return $t['express_code'] == $express_code; });
        ...
        if(!empty($expressinfo)){
            ...
            $deliver_info = Memberorder::_get_express($express_code, $order_info['point_shippingcode'], $orderaddress_info['pointoa_mobphone']);
        }else{
            $express_name='';
            $deliver_info=array();
        }
        ds_json_encode(10000, '', array('express_name' => $express_name, 'shipping_code' => $order_info['point_shippingcode'], 'deliver_info' => $deliver_info));
    }
}
