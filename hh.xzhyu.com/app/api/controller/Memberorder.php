...
class Memberorder extends MobileMember {
    ...
    public function order_list() {
        ...
        $order_list_array = $order_model->getOrderList($condition, 5, '*', 'order_id desc', 0, array('order_common', 'order_goods', 'store'));
        ...
        foreach ($order_list_array as $value) {
            ...
            if (isset($value['extend_order_goods'])) {
                foreach ($value['extend_order_goods'] as $val) {
                    $val['image_240_url'] = goods_cthumb($val['goods_image'], 240, $val['store_id']);
                    $val['image_url'] = goods_cthumb($val['goods_image'], 240, $val['store_id']);
            ...
            if ($value['order_state'] == ORDER_STATE_NEW || $value['order_state'] == ORDER_STATE_DEPOSIT || $value['order_state'] == ORDER_STATE_REST) {
                if (!isset($order_group_list[$value['pay_sn']]['pay_amount'])) {
                    $order_group_list[$value['pay_sn']]['pay_amount'] = 0;
                }
                $order_group_list[$value['pay_sn']]['pay_amount'] += ($value['order_state'] == ORDER_STATE_DEPOSIT?$value['presell_deposit_amount']:($value['order_amount'] - $value['presell_deposit_amount'] + $value['presell_rcb_amount'] + $value['presell_pd_amount'])) - $value['rcb_amount'] - $value['pd_amount'];
            }
    ...
    public function order_cancel() {
        ...
        try{
            Db::startTrans();
            $logic_order->changeOrderStateCancel($order_info, 'buyer', $this->member_info['member_name'], lang('other_reason'));
        } catch (\Exception $e) {
            Db::rollback();
    ...
    public function search_deliver() {
        ...
        if(isset($express[$order_info['extend_order_common']['shipping_express_id']])){
            ...
            $deliver_info = $this->_get_express($express_code, $order_info['shipping_code'], $order_info['extend_order_common']['reciver_info']['phone']);
        }else{
            $express_name='';
            $deliver_info=array();
        }
     ...
     * @apiSuccess {Int} result.order_info.goods_list.goods_type  商品类型 1默认2抢购商品3秒杀商品4组合套装5赠品6拼团7会员等级折扣
    ...
    public function order_info() {
        ...
        if (!empty($order_info['extend_order_common']['invoice_info'])) {
            $order_info['invoice'] = $order_info['extend_order_common']['invoice_info'];
        }
