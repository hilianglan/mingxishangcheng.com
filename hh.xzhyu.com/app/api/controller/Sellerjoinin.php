...
class Sellerjoinin extends MobileMember {
    ...
    public function step4() {
        ...
        //取最新店铺分类信息
        $storeclass_id = intval(input('post.storeclass_id'));
        if ($storeclass_id <= 0) {
            ds_json_encode(10001, lang('select_store_category'));
        }
        $store_class_info = model('storeclass')->getStoreclassInfo(array('storeclass_id' => $storeclass_id));
        if ($store_class_info) {
