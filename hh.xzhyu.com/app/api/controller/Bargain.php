...
class Bargain extends MobileMall {
    ...
    public function get_list() {
        ...
        foreach($bargain_list as $key => $val){
            $goods_info=$goods_model->getGoodsInfoByID($val['bargain_goods_id']);
            if(!$goods_info || $goods_info['goods_state']!=1 || $goods_info['goods_verify']!=1){
                unset($bargain_list[$key]);
                continue;
            }
            $bargain_list[$key]['bargain_goods_image_url'] = goods_cthumb($val['bargain_goods_image'], 480, $val['store_id']);
            //获取已砍数量
            $pbargainorder_count = $pbargainorder_model->getPbargainorderCount(array('bargain_id' => $val['bargain_id'],'bargainorder_state' => 2));
            if($goods_info['goods_storage']){
                $order_percent=$pbargainorder_count/$goods_info['goods_storage']*100;
                if($order_percent>100){
                    $order_percent=100;
                }
            }else{
                $order_percent=100;
            }
            $bargain_list[$key]['order_percent'] = $order_percent;
            $bargain_list[$key]['order_count'] = $pbargainorder_count;
        }
    ...
    public function get_order_info() {
        ...
        $bargainorder_info['bargain_goods_diff_price'] = number_format($bargainorder_info['bargain_goods_price']-$bargainorder_info['bargainorder_current_price'],'2');
