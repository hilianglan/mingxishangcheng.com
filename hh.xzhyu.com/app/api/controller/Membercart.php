...
class Membercart extends MobileMember {
    ...
    public function cart_list() {
        ...
        foreach ($cart_list as $key => $val) {
            ...
            //获取店铺代金券
            $cart_a[$val['store_id']]['voucher_list'] = $voucher_model->getVouchertemplateList(array(array('vouchertemplate_store_id', '=', $val['store_id']), array('vouchertemplate_if_private', '=', 0), array('vouchertemplate_state', '=', 1), array('vouchertemplate_enddate', '>', TIMESTAMP)));
//            $goods_data = $goods_model->getGoodsOnlineInfoForShare($val['goods_id']);
            ...
            $cart_a[$val['store_id']]['goods'][$key]['goods_state'] = $val['state'];
...
//            }
            ...
            if ($val['bl_id']) {
                foreach ($cart_a[$val['store_id']]['goods'][$key]['bl_goods_list'] as $k => $v) {
                    $cart_a[$val['store_id']]['goods'][$key]['bl_goods_list'][$k]['goods_image_url'] = goods_cthumb($v['goods_image'], $val['store_id']);
                }
            }
            if ($val['state']) {
        ...
        ds_json_encode(10000, '', array('cart_list' => $cart_a, 'sum' => ds_price_format($sum), 'cart_count' => count($cart_list), 'cart_val' => $cart_b), request()->action());
    }
    ...
    public function cart_add() {
        if (isset($this->member_info) && !$this->member_info['is_buylimit']) {
            ds_json_encode(10001, lang('cart_buy_noallow'), '', request()->action());
        }
        if (config('ds_config.member_auth') && $this->member_info['member_auth_state'] != 3) {
            ds_json_encode(10001, lang('cart_buy_noauth'), '', request()->action());
        }
        ...
        if (!$bl_id) {
            if ($goods_id <= 0 || $quantity <= 0) {
                ds_json_encode(10001, lang('param_error'), '', request()->action());
            }
            $goods_info = $goods_model->getGoodsOnlineInfoAndPromotionById($goods_id);

            //验证是否可以购买
            if (empty($goods_info)) {
                ds_json_encode(10001, lang('cart_add_goods_not_exists'), '', request()->action());
            }
            ...
            if (isset($this->member_info) && $goods_info['store_id'] == $this->member_info['store_id']) {
                ds_json_encode(10001, lang('cannot_buy_self_goods'), '', request()->action());
            }
            if (intval($goods_info['goods_storage']) < 1 || intval($goods_info['goods_storage']) < $quantity) {
                ds_json_encode(10001, lang('cart_add_stock_shortage'), '', request()->action());
            }
        } else {
            //优惠套装加入购物车(单套)
            ...
            if (empty($bl_info) || $bl_info['bl_state'] == '0') {
                ds_json_encode(10001, lang('recommendations_buy_separately'), '', request()->action());
            }
        ...
        $result = $cart_model->addCart($param, 'db', $quantity);
        if ($result) {
            ds_json_encode(10000, lang('ds_common_op_succ'), $result, request()->action());
        } else {
            ds_json_encode(10001, $cart_model->error_message, '', request()->action());
        }
    }
    ...
    public function cart_del() {
        ...
        ds_json_encode(10000, '', 1, request()->action());
    }
    ...
    public function cart_edit_quantity() {
        ...
        if (!$this->_check_goods_storage($cart_info, $quantity, $this->member_info['member_id'])) {
            ds_json_encode(10001, lang('goods_quantity_limit'), '', request()->action());
        }
        ...
        $condition[] = array('buyer_id','=',$cart_info['buyer_id']);

        $update = $cart_model->editCart($data, $condition,$cart_info['buyer_id']);
        if ($update) {
            $return = array();
            ...
            ds_json_encode(10000, '', $return, request()->action());
        } else {
            ds_json_encode(10001, lang('ds_common_op_fail'), '', request()->action());
        }
    }
    ...
    public function cart_count() {
        ...
        ds_json_encode(10000, '', $data, request()->action());
    }
