...
//                 if (empty($pd_payment_info)) return;
    ...
    /**
     * 漫步 2023 6.14
     * 犀豆（积分）支付款支付,依次循环每个订单
     * 如果犀豆（积分）足够就单独支付了该订单，如果不足就暂时冻结，等API支付成功了再彻底扣除
     */
    public function ptPay($order_list, $input, $buyer_info) {
        // echo 9999;die();
        $member_id = $buyer_info['member_id'];
        $member_name = $buyer_info['member_name'];
...
//                 if (empty($pd_payment_info)) return;
        ...
        $member_points = floatval($buyer_info['member_points']);
        if ($member_points <= 0)
            return;
        ...
        foreach ($order_list as $key => $order_info) {
            ...
            // echo $order_info['order_state'];die();

            //货到付款的订单、已经充值卡、预存款 支付的订单跳过
            if ($order_info['payment_code'] == 'offline')
                continue;
            if ($order_info['order_state'] == ORDER_STATE_PAY)
                continue;
            ...
            if (!isset($order_info['pt_amount'])) {
                $order_list[$key]['pt_amount'] = $order_info['pt_amount'] = 0;
            }
            
            
            
            $order_amount = round(($order_info['order_state'] == ORDER_STATE_DEPOSIT?$order_info['presell_deposit_amount']:($order_info['order_amount'] - $order_info['presell_deposit_amount'] + $order_info['presell_rcb_amount'] + $order_info['presell_pd_amount']+$order_info['presell_pt_amount'])) - $order_info['rcb_amount'] - $order_info['pd_amount']- $order_info['pt_amount'], 2);
...
// echo $order_amount;die();
            ...
            if ($member_points > 0 && $order_amount>0) {
                if ($member_points >= $order_amount) {
                    $data_pd['amount'] = $order_amount;
                    $member_points -= $order_amount;
                    $canPay=true;
                } else {
                    $data_pd['amount'] = $member_points;
                    $member_points = 0;
                    $canPay=false;
                }
                $predeposit_model->changePt('order_freeze', $data_pd);
                //预存款支付金额保存到订单
                $data_order = array();
                $order_list[$key]['pt_amount'] = $data_order['pt_amount'] = round($order_info['pt_amount'] + $data_pd['amount'], 2);
                $result = $order_model->editOrder($data_order, array('order_id' => $order_info['order_id']));
