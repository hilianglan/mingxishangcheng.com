...
class Predeposit extends BaseModel {
     ...
     * @return type
     */
    public function changePt($change_type, $data = array()) {
        $data_log = array();
        $data_pd = array();
        $data_msg = array();
        ...
        switch ($change_type) {
            case 'order_pay':
                $data_log['lg_av_amount'] = -$data['amount'];
                $data_log['lg_desc'] = '下单，支付预存款，订单号: ' . $data['order_sn'];
                $data_pd['member_points'] = Db::raw('member_points-'.$data['amount']);
            ...
            case 'order_freeze':
                ...
                $data_log['lg_freeze_amount'] = $data['amount'];
                $data_log['lg_desc'] = '下单，冻结预存款，订单号: ' . $data['order_sn'];
                $data_pd['freeze_points'] = Db::raw('freeze_points+'.$data['amount']);
                $data_pd['member_points'] = Db::raw('member_points-'.$data['amount']);
        ...
        // print_r($data_pd);print_r($data);die();

        $update = model('member')->editMember(array('member_id' => $data['member_id']), $data_pd,$data['member_id']);

        if (!$update) {
            throw new \think\Exception('操作失败1', 10006);
        }
        $insert = Db::name('pdlog')->insertGetId($data_log);
        if (!$insert) {
            throw new \think\Exception('操作失败2', 10006);
        }
